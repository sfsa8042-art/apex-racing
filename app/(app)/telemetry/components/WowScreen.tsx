...
function ProfileBadge({ profile }: { profile: DriverProfile }) {
  ...
  return (
    <div className={cn("inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-xs font-mono", colors[profile.style])}>
      ...
      <span>{profile.styleLabel} Driver</span>
    </div>
  );
}
  ...
  return (
    <div className={cn("rounded-lg border p-3", colors[idx] ?? colors[2])}>
      <div className="flex items-start justify-between gap-2 mb-1">
        ...
        <span className="text-sm font-mono font-bold shrink-0">−{issue.lossS.toFixed(3)}s</span>
      </div>
      {issue.academyLink && (
        <a href={issue.academyLink}
          ...
          <BookOpen size={10} />
          {issue.academyTitle ?? "Learn the fix"} →
        </a>
      )}
    </div>
  );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-950/90 backdrop-blur-sm p-4" style={{animation:"fadeIn 0.25s ease both"}}>
      <div className="w-full max-w-xl bg-zinc-900 border border-zinc-700 rounded-2xl shadow-2xl overflow-hidden" style={{animation:"modalIn 0.4s cubic-bezier(0.34,1.56,0.64,1) both"}}>
        ...
        <div className="px-6 pt-6 pb-4 border-b border-zinc-800">
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1 min-w-0">
              <p className="text-[10px] font-mono uppercase tracking-widest text-zinc-500 mb-2">Lap Analysed</p>
          ...
          <div className="flex items-center gap-5 mt-4 flex-wrap">
            <div>
              <p className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest">Your lap</p>
              <p className="text-xl font-mono tabular font-semibold text-lime-400">{lapTimeStr}</p>
            </div>
            <div>
              <p className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest">vs Reference</p>
              <p className={cn("text-xl font-mono tabular font-semibold flex items-center gap-1",
                ...
                {summary.totalLossS > 0.05
                  ? <><TrendingDown size={16} /> +{summary.totalLossSFormatted}s</>
                  : <><TrendingUp size={16} /> Ahead!</>
                }
              </p>
            </div>
            <div>
              <p className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest">Potential</p>
              <p className="text-xl font-mono tabular font-semibold text-yellow-400">
                −{summary.potentialGainS.toFixed(3)}s
              </p>
            </div>
          ...
          <div className="px-6 pb-2">
            <div className="rounded-lg border border-lime-400/20 bg-lime-400/5 px-3 py-2">
              <p className="text-xs text-lime-400 font-medium mb-1">
                ✓ Good work on {summary.goodAreas.join(" and ")}
              </p>
        ...
        <button onClick={() => setShowDetails((v) => !v)}
          ...
          <span className="font-mono">Driver profile details</span>
        ...
        {showDetails && (
          <div className="px-6 pb-4 animate-slide-up">
            <div className="grid grid-cols-2 gap-2">
              {[
                { label: "Brake confidence", value: summary.profile.brakeConfidence },
                { label: "Throttle control",  value: summary.profile.throttleControl },
                { label: "Corner speed",      value: summary.profile.cornerSpeed },
                { label: "Consistency",       value: summary.profile.consistency },
              ].map(({ label, value }) => (
